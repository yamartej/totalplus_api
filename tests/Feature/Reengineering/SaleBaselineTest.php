...
class SaleBaselineTest extends TestCase
{
    ...
    private function grantApiPermissions(
        ...
        $role = Roles::firstOrCreate(
            ...
            ['description' => 'Test-only sales characterization role']
        );
        ...
        $permissionIds = collect($permissionNames)
            ->map(function (string $name) {
                return Permission::firstOrCreate(
                    ['name' => $name],
                    ['description' => 'Sales test permission']
                )->id;
            })
            ->all();
    ...
    private function authHeaders(User $user): array
    {
        ...
        return [
            'Authorization' => 'Bearer '
                . $user
                    ->createToken('phase-3-sales-test')
                    ->plainTextToken,
        ];
    }

    public function test_sale_contract_creates_sale_details_and_inventory_issues(): void
    {
        $ctx = $this->context();
        ...
        $payload = [
            ...
            'carts' => [
                [
                    'productId' => $ctx['productA']->id,
                    'quantity' => 2,
                ],
                [
                    'productId' => $ctx['productB']->id,
                    'quantity' => 1,
                ],
            ],
        ];
        ...
        $this->assertSame(
            8,
            (int) $ctx['inventoryA']->fresh()->quantity
        );
        $this->assertSame(
            9,
            (int) $ctx['inventoryB']->fresh()->quantity
        );
    ...
    public function test_backend_calculates_total_instead_of_trusting_client(): void
    {
        $ctx = $this->context();
    ...
    public function test_sale_rejects_insufficient_stock_and_rolls_back(): void
    {
    ...
    public function test_multi_product_sale_void_restores_each_product_correctly(): void
    {
